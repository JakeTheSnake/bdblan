// Highest net worth any single LAN player reached in one game, plus the hero
// they played in that game.
import { getDb } from '../db.js';

/**
 * @param {number} lanId
 * @returns {Promise<Map<number, { value: number, hero_id: number, hero_icon: string|null }>>}
 *   accountId -> best single-game net worth and the hero played
 */
export async function getHighestNetWorthForLan(lanId) {
  ...
  for (const r of rows) {
    ...
    const value = Number(r.net_worth) || 0;
    const prev = result.get(accountId);
    if (!prev || value > prev.value) {
      result.set(accountId, { value, hero_id: r.hero_id, hero_icon: r.hero_icon || null });
    }
  }
  return result;
}
