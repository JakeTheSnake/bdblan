...
export async function getMatchDetails(lanId, matchId) {
  ...
  const towers = objectives
    ...
    .map((o) => {
      // key_name like "npc_dota_goodguys_tower1_mid" — goodguys = radiant side.
      // The tower belongs to that side; the killer is the other side.
      const key = o.key_name || '';
      const victimIsRadiant = key.includes('goodguys');
      const killerIsRadiant = !victimIsRadiant;
      return { time: o.time, key, is_us: killerIsRadiant === ourRadiant };
    });
