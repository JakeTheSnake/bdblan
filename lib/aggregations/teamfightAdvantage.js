// For each teamfight in a LAN's matches, decide whether "our team" had less
// participating heroes than the enemy, split by matches we won vs lost — to see
// whether teamfight numbers disadvantage correlates with losing.
 ...
 * @param {number} lanId
 * @returns {Promise<{
 *   wins: { fights: number, disadvantage: number },
 *   losses: { fights: number, disadvantage: number },
 * }>}
 */
export async function getTeamfightDisadvantageForLan(lanId) {
  const db = getDb();
  const empty = { wins: { fights: 0, disadvantage: 0 }, losses: { fights: 0, disadvantage: 0 } };
  ...
  const wins = { fights: 0, disadvantage: 0 };
  const losses = { fights: 0, disadvantage: 0 };
  ...
  for (const row of matchRows) {
    ...
    for (const tf of teamfights) {
      ...
      bucket.fights += 1;
      if (theirCount > ourCount) bucket.disadvantage += 1;
    }
  }
