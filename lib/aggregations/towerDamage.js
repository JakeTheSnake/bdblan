// Total tower damage per LAN player.
import { sql } from 'kysely';
import { getDb } from '../db.js';

/**
 * @param {number} lanId
 * @returns {Promise<Map<number, number>>} accountId -> total tower damage
 */
export async function getTowerDamageForLan(lanId) {
  const db = getDb();
  ...
  const rows = await db
    ...
    .select([
      ...
      sql`SUM(COALESCE(match_players.tower_damage, 0))`.as('total_tower_damage'),
    ])
  ...
  for (const r of rows) {
    result.set(Number(r.account_id), Number(r.total_tower_damage));
  }
  return result;
}
